...
\end{figure}
pnpjs \cite{Pnpjs} easier communication with SharePoint (requires a setup function to be executed in the "main.ts" file which defines all accepted headers of the requests and responses), needs import '@pnp/polyfill-ie11'; polyfill
anstatt JSOM \cite{JSOM}, CSOM \cite{CSOM} oder SharePoint REST API \cite{SPRest} => "sp" \cite{PnpjsSp} the fluent api wenn pnpjs verwendet werden kann, 
selbstgebautes Backend "wcf data service" \cite{SPWcf} API mit Angulars "HttpClient",
SharePointHttpClient wenn extra Authenfizierung benötigt ist um Daten zu beanspruchen \cite{SPRest},
SharePoint Script On Demand \cite{SpSod}
services, service entities mapping and entities
